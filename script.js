...
async function loadProjects() {
  try {
    ...
    data.forEach(p => {
      ...
      let firstImg = p.screenshots?.[0] || p.images?.[0] || 'images/default_thumb.jpg';
      thumb.src = firstImg;
...
function openModal(project) {
  ...
  document.getElementById('modal-long').textContent = project.long || project.short || '';
  ...
  currentCarouselImages = project.screenshots?.length ? project.screenshots : (project.images || []);
...
document.addEventListener('click', e => {
  if (e.target.matches('#modal-close') || e.target.matches('.proj-modal .modal-overlay')) closeModal();
  if (e.target.matches('#iv-close') || e.target.matches('.img-viewer .iv-overlay')) closeImageViewer();
});
